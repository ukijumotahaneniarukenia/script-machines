...
ENV_HOME_DIR=/home/aine

ENV_RC_FILE_NAME=.bashrc
...
eval echo {$START_HOST_NO..$END_HOST_NO} | xargs -n1|nl|\
  while read n h;do
		...
		echo -e "echo \x27export MACHINE_NAME=$REPLICA_NAME-$(printf  $SUBGRP_DIGIT $[n])\x27 >> $DEPLOY_DIR/$REPLICA_NAME-$(printf  $SUBGRP_DIGIT $[n])$ENV_HOME_DIR/$ENV_RC_FILE_NAME"
		...
		echo -e "echo \x27export MACHINE_IP=$MACHINE_IP\x27 >> $DEPLOY_DIR/$REPLICA_NAME-$(printf  $SUBGRP_DIGIT $[n])$ENV_HOME_DIR/$ENV_RC_FILE_NAME"
